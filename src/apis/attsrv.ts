...
const optionsToFlags = (options: Readonly<Record<string, boolean>>) =>
  Object.entries(options).filter(last).map(head).join(',')
...
const attendeeDtoFromRegistrationInfo = (
  registrationInfo: RegistrationInfo
): AttendeeDto => {
  const packagesMap = new Map<string, number>()

  // first copy all original packages
  registrationInfo.originalPackages?.forEach((entry) =>
  ...
  packagesMap.set(
    ...
    registrationInfo.ticketType.type === 'day' &&
      registrationInfo.ticketType.day.weekday === Weekdays.Monday
      ? 1
      : 0
  )
  packagesMap.set(
    'day-tue',
    registrationInfo.ticketType.type === 'day' &&
      registrationInfo.ticketType.day.weekday === Weekdays.Tuesday
      ? 1
      : 0
  )
  packagesMap.set(
    'day-wed',
    registrationInfo.ticketType.type === 'day' &&
      registrationInfo.ticketType.day.weekday === Weekdays.Wednesday
      ? 1
      : 0
  )
  packagesMap.set(
    'day-thu',
    registrationInfo.ticketType.type === 'day' &&
      registrationInfo.ticketType.day.weekday === Weekdays.Thursday
      ? 1
      : 0
  )
  packagesMap.set(
    'day-fri',
    registrationInfo.ticketType.type === 'day' &&
      registrationInfo.ticketType.day.weekday === Weekdays.Friday
      ? 1
      : 0
  )
  packagesMap.set(
    'day-sat',
    registrationInfo.ticketType.type === 'day' &&
      registrationInfo.ticketType.day.weekday === Weekdays.Saturday
      ? 1
      : 0
  )
  packagesMap.set(
    'day-sun',
    registrationInfo.ticketType.type === 'day' &&
      registrationInfo.ticketType.day.weekday === Weekdays.Sunday
      ? 1
      : 0
  )
  packagesMap.set(
  ...
  packagesMap.set(
    ...
    registrationInfo.ticketLevel.level && // it cannot be null here
      !(
        config.ticketLevels[
          registrationInfo.ticketLevel.level
        ].includes?.includes('stage-pass') ?? false
      ) &&
  ...
  packagesMap.set(
    ...
    registrationInfo.ticketLevel.level && // it cannot be null here
      !(
        config.ticketLevels[
          registrationInfo.ticketLevel.level
        ].includes?.includes('tshirt') ?? false
      ) &&
  ...
  packagesMap.set(
    'early',
    registrationInfo.ticketLevel.addons.early.selected ? 1 : 0
  )
  packagesMap.set(
    'late',
    registrationInfo.ticketLevel.addons.late.selected ? 1 : 0
  )
  ...
  packagesMap.set(
    'fursuit',
    registrationInfo.ticketLevel.addons.fursuit.selected ? 1 : 0
  )
  ...
  return {
    ...
    tshirt_size: tshirtToApi(
      registrationInfo.ticketLevel.addons.tshirt.options.size
    ),
...
// eslint-disable-next-line complexity
const registrationInfoFromAttendeeDto = (
  attendeeDto: AttendeeDto
): RegistrationInfo => {
  const packagesMap = new Map(
    attendeeDto.packages_list.map((entry) => [entry.name, entry.count])
  )
  ...
  const addons = {
    ...
    'stage-pass': {
      selected:
        (config.ticketLevels[level].includes?.includes('stage-pass') ??
          false) ||
    ...
    tshirt: {
      selected:
        (config.ticketLevels[level].includes?.includes('tshirt') ?? false) ||
        packagesMap.has('tshirt'),
      options: {
        size: tshirtFromApi(
          attendeeDto.tshirt_size
        ) as RegistrationInfo['ticketLevel']['addons']['tshirt']['options']['size'],
      },
    },
    fursuit: {
  ...
  if (packagesMap.has('benefactor')) {
    addons.benefactor = {
      ...
      options: {
        count:
          `c${packagesMap.get('benefactor')}` as RegistrationInfo['ticketLevel']['addons']['benefactor']['options']['count'],
      },
    }
  }
  ...
  if (packagesMap.has('fursuitadd')) {
    addons.fursuitadd = {
      ...
      options: {
        count:
          `c${packagesMap.get('fursuitadd')}` as RegistrationInfo['ticketLevel']['addons']['fursuitadd']['options']['count'],
      },
    }
  }
  ...
  return {
    ...
    ticketType: packagesMap.has('attendance')
      ...
      : {
          type: 'day',
          day: packagesMap.has('day-sun')
            ? days.find((d) => d.weekday === Weekdays.Sunday)!
            : packagesMap.has('day-mon')
              ? days.find((d) => d.weekday === Weekdays.Monday)!
              : packagesMap.has('day-tue')
                ? days.find((d) => d.weekday === Weekdays.Tuesday)!
                : packagesMap.has('day-wed')
                  ? days.find((d) => d.weekday === Weekdays.Wednesday)!
                  : packagesMap.has('day-thu')
                    ? days.find((d) => d.weekday === Weekdays.Thursday)!
                    : packagesMap.has('day-fri')
                      ? days.find((d) => d.weekday === Weekdays.Friday)!
                      : packagesMap.has('day-sat')
                        ? days.find((d) => d.weekday === Weekdays.Saturday)!
    ...
    contactInfo: {
      ...
      country:
        attendeeDto.country as RegistrationInfo['contactInfo']['country'],
    },
    optionalInfo: {
...
export class AttSrvAppError extends AppError<
  Replace<ErrorMessage, '.', '-', { all: true }>
> {
  ...
  constructor(err: AjaxError) {
    ...
    super(
      ...
      `Attendee API Error: ${JSON.stringify(errDto, undefined, 2)}`
    )
  }
}
...
const apiCall = <T>({
  ...
  ajax<T>({
    ...
    ...cfg,
  }).pipe(catchError(handleStandardApiErrors(AttSrvAppError)))
 ...
 */
export const registrationCountdownCheck = () =>
  apiCall<CountdownDto>({
    path: '/countdown',
    method: 'GET',
  })
 ...
 * This endpoint is optimized in the backend for high traffic, so it is safe to call during initial reg.
 */
export const submitRegistration = (registrationInfo: RegistrationInfo) =>
  apiCall({
    path: '/attendees',
    method: 'POST',
 ...
 */
export const findMyRegistrations = () =>
  apiCall<AttendeeIdListDto>({
    path: '/attendees',
    method: 'GET',
  })
 ...
 * 500: It is important to communicate the ErrorDto's requestid field to the user, so they can give it to us, so we can look in the logs.
 */
export const loadRegistration = (id: number) =>
  apiCall<AttendeeDto>({
    path: `/attendees/${id}`,
    method: 'GET',
  })
 ...
 * 500: It is important to communicate the ErrorDto's requestid field to the user, so they can give it to us, so we can look in the logs.
 */
export const loadRegistrationStatus = (id: number) =>
  apiCall<AttendeeStatusDto>({
...
export const findExistingRegistration = () =>
  findMyRegistrations().pipe(
    ...
    catchError((err) => {
      if (
        err instanceof AttSrvAppError &&
        err.code === 'attendee-owned-notfound'
      ) {
        return of(undefined)
      } else {
        throw err
