...
export type RegistrationStatus = 'new' | 'approved' | 'partially paid' | 'paid' | 'checked in' | 'cancelled' | 'waiting'
...
const attendeeDtoFromRegistrationInfo = (registrationInfo: RegistrationInfo): AttendeeDto => {
    ...
    packagesMap.set(
        'day-mon',
        registrationInfo.ticketType.type === 'day' && registrationInfo.ticketType.day.weekday === Weekdays.Monday
            ? 1
            : 0,
    )
    packagesMap.set(
        'day-tue',
        registrationInfo.ticketType.type === 'day' && registrationInfo.ticketType.day.weekday === Weekdays.Tuesday
            ? 1
            : 0,
    )
    packagesMap.set(
        'day-wed',
        registrationInfo.ticketType.type === 'day' && registrationInfo.ticketType.day.weekday === Weekdays.Wednesday
            ? 1
            : 0,
    )
    packagesMap.set(
        'day-thu',
        registrationInfo.ticketType.type === 'day' && registrationInfo.ticketType.day.weekday === Weekdays.Thursday
            ? 1
            : 0,
    )
    packagesMap.set(
        'day-fri',
        registrationInfo.ticketType.type === 'day' && registrationInfo.ticketType.day.weekday === Weekdays.Friday
            ? 1
            : 0,
    )
    packagesMap.set(
        'day-sat',
        registrationInfo.ticketType.type === 'day' && registrationInfo.ticketType.day.weekday === Weekdays.Saturday
            ? 1
            : 0,
    )
    packagesMap.set(
        'day-sun',
        registrationInfo.ticketType.type === 'day' && registrationInfo.ticketType.day.weekday === Weekdays.Sunday
            ? 1
            : 0,
    )
...
const registrationInfoFromAttendeeDto = (attendeeDto: AttendeeDto): RegistrationInfo => {
    ...
    const addons = {
        ...
        'stage-pass': {
            selected:
                (config.ticketLevels[level].includes?.includes('stage-pass') ?? false) || packagesMap.has('stage'),
            options: {},
        },
        tshirt: {
 ...
 * 500: It is important to communicate the ErrorDto's requestid field to the user, so they can give it to us, so we can look in the logs.
 */
export const updateRegistration = (id: number, registrationInfo: RegistrationInfo) =>
    apiCall({
        path: `/attendees/${id}`,
        method: 'PUT',
        body: attendeeDtoFromRegistrationInfo(registrationInfo),
