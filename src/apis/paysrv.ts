    ...
    findTransactionsForBadgeNumber(badgeNumber).pipe(
        concatMap((transactions) => {
            const tentativeTransaction = transactions.find(
                (t) => t.transaction_type === 'payment' && t.method === 'credit' && t.status === 'tentative',
            )
...
export const initiateSepaPaymentOrUseExisting = (badgeNumber: number) =>
    findTransactionsForBadgeNumber(badgeNumber).pipe(
        concatMap((transactions) => {
            const tentativeTransaction = transactions.find(
                (t) => t.transaction_type === 'payment' && t.method === 'transfer' && t.status === 'tentative',
            )

            return tentativeTransaction !== undefined ? of(tentativeTransaction) : initiateSepaPayment(badgeNumber)
        }),
    )
