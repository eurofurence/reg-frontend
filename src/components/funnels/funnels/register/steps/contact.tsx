...
const Contact = (_: ReadonlyRouteComponentProps) => {
  ...
  const { countryOptions, countryOptionsByValue } = useMemo(() => {
    const countryNames = sortBy(
      prop('name'),
      config.allowedCountries.map((countryCode) => ({
        countryCode,
        name: l10n.getString('country-name', { countryCode }),
      }))
    )
  ...
  return (
    <WithInvoiceRegisterFunnelLayout onNext={handleSubmit} currentStep={3}>
      ...
      <Form onSubmit={handleSubmit}>
        <Localized
          id="register-contact-info-email"
        ...
        >
          <TextField
            ...
            {...register('email', {
              ...
              maxLength: 200,
              pattern: reEmail,
              validate: { isVerified: (v) => verifiedEmails.includes(v!) },
            })}
          />
        </Localized>
        <Localized
          id="register-contact-info-phone-number"
          attrs={{ label: true, placeholder: true }}
        >
          <TextField
            label="Phone number"
            ...
            {...register('phoneNumber', { required: true, maxLength: 32 })}
          />
        </Localized>
        <Localized
          id="register-contact-info-telegram-username"
          attrs={{ label: true, placeholder: true }}
        >
          <TextField
        ...
        <Localized
          id="register-contact-info-street"
          attrs={{ label: true, placeholder: true }}
        >
          <TextField
            label="Street"
            ...
            {...register('street', { required: true, maxLength: 120 })}
          />
        </Localized>
        <Localized
          id="register-contact-info-postal-code"
          attrs={{ label: true, placeholder: true }}
        >
          <TextField
        ...
        <Localized
          id="register-contact-info-state-or-province"
          attrs={{ label: true, placeholder: true }}
        >
          <TextField
            ...
            {...register('stateOrProvince', { maxLength: 80 })}
          />
        </Localized>
        <FunnelController
          control={control}
          ...
          render={({ field: { onChange, value, ref, ...field } }) => (
            <Localized
              id="register-contact-info-country"
              attrs={{ label: true }}
            >
              <Select
                label="Country"
