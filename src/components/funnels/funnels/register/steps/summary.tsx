...
const Section = ({ id: sectionId, editLink, properties }: SectionProps) => {
    ...
    return (
        <SectionContainer status={status}>
            <Localized id={`register-summary-section-${sectionId}-title`}>
                <SectionTitle>{sectionId}</SectionTitle>
            </Localized>
            {status === 'cancelled' ? undefined : (
                <Localized id="register-summary-edit">
                    <Link css={editButtonStyle} to={editLink}>
                        Edit information
                    </Link>
                </Localized>
            )}
            <PropertyList>
                {properties.map(({ id, value, wide = false }) => (
                    <Property key={id} wide={wide}>
                        <Localized id={`register-summary-section-${sectionId}-property-${id}-name`}>
...
const Summary = (_: ReadonlyRouteComponentProps) => {
    ...
    const {
        ...
        register,
        formState: { errors },
    } = useFunnelForm('register-summary')
        ...
        <WithInvoiceRegisterFunnelLayout onNext={handleSubmit} currentStep={5}>
            ...
            <Localized id="register-summary-registration-status" vars={{ status }}>
                <StatusText status={status}>
                    We have received your registration and will confirm it when things are ready. Keep an eye on your
                    mailbox!
                </StatusText>
            </Localized>
            ...
            <Section
                ...
                properties={[
                    ...
                    {
                        id: 'date-of-birth',
                        value: personalInfo.dateOfBirth.setLocale(locale).toLocaleString({ dateStyle: 'long' }),
                    },
                    {
                        id: 'wheelchair-accomodation',
                        value: l10n.getString('register-summary-boolean-value', {
                            value: personalInfo.wheelchair.toString(),
                        }),
                    },
                    {
                        id: 'spoken-languages',
                        ...
                        value: personalInfo.spokenLanguages
                            .map((languageCode) =>
                                l10n.getString('language-name', { languageCode }, langmap[languageCode].englishName),
                            )
                            .join(', '),
                    },
                ]}
            ...
            <Section
                id="contact"
                editLink="/register/contact-info"
                properties={[
                    ...
                    { id: 'state-or-province', value: contactInfo.stateOrProvince ?? '' },
                    {
                        ...
                        value: l10n.getString(
                            'country-name',
                            { countryCode: contactInfo.country },
                            contactInfo.country,
                        ),
                    },
                ]}
            ...
            <Section
                id="optional"
                editLink="/register/optional-info"
                properties={[
                    { id: 'notifications', wide: true, value: notificationNames },
                    {
                        ...
                        wide: true,
                        value: l10n.getString('register-summary-boolean-value', {
                            value: optionalInfo.digitalConbook.toString(),
                        }),
                    },
            ...
            {isEdit ? undefined : (
                <TermsForm onSubmit={handleSubmit}>
                    <Checkbox gridSpan={10} {...register('rulesAndConditionsAccepted', { required: true })}>
                        <Localized
                            id="register-summary-rules-and-conditions-accepted"
                            elems={{
                    ...
                    {errors.rulesAndConditionsAccepted?.message === undefined ? undefined : (
                        <ErrorMessage>{errors.rulesAndConditionsAccepted.message}</ErrorMessage>
                    )}
                </TermsForm>
            )}
