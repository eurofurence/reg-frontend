...
const StatusText = styled.p<{ readonly status: RegistrationStatus }>`
	color: ${({ status }) =>
		status === "cancelled" ? "var(--color-semantic-error)" : "unset"};
`
			...
			<PropertyList>
				{properties.map(({ id, value, wide = false }) => (
					<Property key={id} wide={wide}>
						<Localized
							id={`register-summary-section-${sectionId}-property-${id}-name`}
						>
		...
		<WithInvoiceRegisterFunnelLayout onNext={handleSubmit} currentStep={5}>
			...
			<Localized id="register-summary-registration-status" vars={{ status }}>
				<StatusText status={status}>
					We have received your registration and will confirm it when things are
					ready. Keep an eye on your mailbox!
				</StatusText>
			</Localized>

			{registrationId ? (
				<Localized
					id="register-summary-registration-id"
					vars={{ registrationId }}
				>
						...
						value: personalInfo.spokenLanguages
							.map((languageCode) =>
								l10n.getString(
									"language-name",
									{ languageCode },
									langmap[languageCode].englishName,
								),
							)
							.join(", "),
					},
			...
			{isEdit ? undefined : (
				<TermsForm onSubmit={handleSubmit}>
					<Checkbox
						gridSpan={10}
						{...register("rulesAndConditionsAccepted", { required: true })}
					>
						<Localized
							...
							elems={{
								rules: (
									<a
										...
										href={config.websiteLinks.rules}
									/>
								),
								conditions: (
									<a
										target="_blank"
										rel="noreferrer noopener"
										href={config.websiteLinks.terms}
									/>
								),
							}}
						>
