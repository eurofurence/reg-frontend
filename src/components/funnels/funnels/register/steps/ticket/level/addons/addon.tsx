...
const TicketLevelAddon = ({ addon, formContext }: TicketLevelAddonProps) => {
    const isIncluded = (lvl: TicketLevel['level'] | null) =>
        lvl !== null && (config.ticketLevels[lvl].includes?.includes(addon.id) ?? false)
    const isRequired = (lvl: TicketLevel['level'] | null) =>
        lvl !== null && (config.ticketLevels[lvl].requires?.includes(addon.id) ?? false)
    const isUnavailable = (lvl: TicketLevel['level'] | null) =>
        lvl !== null && (config.addons[addon.id].unavailableFor?.level?.includes(lvl) ?? false)
        ...
        const subscription = watch((value, { name, type }) => {
            if (resetOnLevelChange && name === 'level' && type === 'change') {
                ...
                setValue(
                    ...
                    ((isIncluded(levelValue) || isRequired(levelValue)) && !isUnavailable(levelValue)) || addon.default,
                )
            }
            ...
            if (name) {
                if (name.startsWith('addons') && type === 'change') {
                    ...
                    if (requirements.length > 0) {
                        const unselectedAddonIds = Object.entries(value.addons as AddonSelection)
    ...
    return (
        <Localized id={`register-ticket-level-addons-item-${addon.id}`} attrs={{ label: true, description: true }}>
