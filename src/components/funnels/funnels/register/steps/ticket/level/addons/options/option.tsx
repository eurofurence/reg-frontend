...
type FilterByType<
  T extends string = AllOptionDefs['type'],
  U = AllOptionDefs,
> = Extract<U, { readonly type: T }>

export type AugmentedOption<T extends string = AllOptionDefs['type']> = {
  [K in keyof typeof config.addons]: {
    [L in keyof (typeof config.addons)[K]['options']]: FilterByType<
      T,
      (typeof config.addons)[K]['options'][L]
    > & {
      readonly addonId: K
      readonly id: L
    }
