  ...
  const { items, itemsByValue } = useMemo(() => {
    const items = option.items.map((value) => ({
      ...
      label: l10n.getString(
        `register-ticket-level-addons-item-${option.addonId}-option-${option.id}-value`,
        { value }
      ),
    }))
    ...
    return {
      items,
      itemsByValue: new Map(items.map((item) => [item.value, item])),
    }
  }, [l10n])

  const addonErrorOptions = errors.addons?.[option.addonId]
    ?.options as AddonErrorOptions
  ...
  return (
    <FunnelController
      ...
      render={({ field: { onChange, value, ref, ...field } }) => (
        <Localized
          id={`register-ticket-level-addons-item-${option.addonId}-option-${option.id}`}
          attrs={{ label: true }}
        >
          <Select
            label={option.id}
            ...
            options={items}
            onChange={(item) =>
              !preventChange ? onChange(item?.value) : undefined
            }
