...
const InvoiceTotalItem = ({ type, name, value, extra, warn }: InvoiceTotalItemProps) => (
    <Container warn={warn}>
        <Localized id="invoice-total-item-label" vars={{ name }}>
            <Label>{name}</Label>
        </Localized>
        <Localized id={type} vars={{ value: new FluentNumber(value, { style: 'currency', currency: 'EUR' }) }}>
            <Price>{value} €</Price>
        </Localized>
