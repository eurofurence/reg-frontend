...
import type { ReadonlyDateTime } from '~/util/readonly-types'
...
const Clock = ({ deadline }: ClockProps) => {
  ...
  const tick$ = useObservable(
    () => interval(CLOCK_UPDATE_DELAY).pipe(O.map(getTimeRemaining)),
    []
  )
  ...
  return (
    <ClockContainer>
      <Localized id="header-clock-component-months" attrs={{ caption: true }}>
        <Display
          caption="Months"
          content={timeRemaining.months!.toString()}
          size={2}
          padding="0"
        />
      </Localized>
      <Localized id="header-clock-component-days" attrs={{ caption: true }}>
        <Display
          caption="Days"
          content={timeRemaining.days!.toString()}
          size={2}
          padding="0"
        />
      </Localized>
      <Localized id="header-clock-component-hours" attrs={{ caption: true }}>
        <Display
          caption="Hours"
          content={timeRemaining.hours!.toString()}
          size={2}
          padding="0"
        />
      </Localized>
      <Localized id="header-clock-component-minutes" attrs={{ caption: true }}>
        <Display
          caption="Minutes"
          content={timeRemaining.minutes!.toString()}
          size={2}
          padding="0"
        />
      </Localized>
      <Localized id="header-clock-component-seconds" attrs={{ caption: true }}>
        <Display
          caption="Seconds"
          content={timeRemaining.seconds!.toString()}
          size={2}
          padding="0"
        />
      </Localized>
...
const friendlyLocales = Object.entries(
  groupBy((locale) => locale.split('-')[0], supportedLanguages)
...
const Header = ({ deadline }: HeaderProps) => {
  ...
  return (
    <NavBar>
      ...
      <NavBarMenu>
        <Localized id="header-menu-item-my-account" attrs={{ label: true }}>
          <NavBarMenuItem
            icon={user}
            label="My account"
          ...
          <NavBarSubMenu icon={globe} label="Language">
            {friendlyLocales.map(({ name, locale }) => (
              <NavBarMenuItem
                key={locale}
                label={name}
                onClick={() => dispatch(SetLocale.create(locale))}
              />
            ))}
          </NavBarSubMenu>
        </Localized>
