...
> = Partial<{
    ...
    validate:
        ...
        | Record<string, LocalizedValidate<FieldPathValue<TFieldValues, TFieldName>>>
}>
...
const loadCache = <F extends FormIds>(id: F) => load<FormCache>('funnel-form')?.[id]
...
export const useFunnelForm = <F extends FormIds>(id: F) => {
    ...
    const { handleSubmit, watch, register, ...methods } = useForm<FormValuesType<F>>({
        defaultValues: (submittedValues ?? autosaveValues.current ?? defaultValues) as never,
    })
    ...
    >({
        rules,
        ...props
    }: Omit<ControllerProps<TFieldValues, TName>, 'rules'> & { rules: LocalizedRules<TFieldValues, TName> }) => {
        const { l10n } = useLocalization()
