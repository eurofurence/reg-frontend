...
const loadUnsubmittedRegistration = () =>
    of(
        LoadRegistrationState.create({
            isOpen: true,
            registration: { status: 'unsubmitted', registrationInfo: loadAutosave()?.registrationInfo ?? {} },
        }),
    )
        ...
        concatMap((reg) => {
            if (reg === undefined) {
                ...
                return of(
                    LoadRegistrationState.create({
                        isOpen: true,
                        registration: { id: reg.id, status: reg.status, registrationInfo: reg.registrationInfo },
                    }),
                )
            } else {
...
export default combineEpics<GetAction<AnyAppAction>, GetAction<AnyAppAction>, AppState>(
    ...
    nextPage(
        ...
        ({ payload }) => `/register/ticket/${payload.type === 'full' ? 'level' : 'day'}`,
    ),
        ...
        action$.pipe(
            ...
            concatMap(([, state]) => {
                ...
                return submitRegistration({
                    ...registrationInfo,
                    preferredLocale: registrationInfo.preferredLocale ?? getDefaultLocale(),
    ...
    (action$, state$) =>
        action$.pipe(
            ...
            withLatestFrom(state$),
            concatMap(([, state]) =>
                initiateCreditCardPaymentOrUseExisting(getRegistrationId()(state)!).pipe(
                    justDo((transaction) => {
    ...
    (action$, state$) =>
        action$.pipe(
            ...
            concatMap(([, state]) =>
                updateRegistration(getRegistrationId()(state)!, getRegistrationInfo()(state)! as RegistrationInfo).pipe(
                    ignoreElements(),
