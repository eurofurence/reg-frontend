...
type ParseAddonOption<T> = T extends { readonly type: 'select'; readonly items: readonly (infer I)[] } ? I : never
...
export type TicketType = { readonly type: 'full' } | { readonly type: 'day'; readonly day: ReadonlyDateTime }
...
export const isUnsubmitted = (r: Registration): r is UnsubmittedRegistration => r.status === 'unsubmitted'

export const isSubmitted = (r: Registration): r is SubmittedRegistration => r.status !== 'unsubmitted'

export const isPending = (r: Registration): r is PendingRegistration => includes(['new', 'waiting'] as const, r.status)
