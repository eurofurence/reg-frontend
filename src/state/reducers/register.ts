    ...
    return {
        addons: Object.fromEntries(
            Object.entries(addons).map(([id, { selected, ...rest }]) => [
                ...
                {
                    selected:
                        !(config.addons[id].unavailableFor?.type?.includes(ticketType.type) ?? false) &&
                        !(config.addons[id].unavailableFor?.level?.includes(payload.level) ?? false) &&
                        !(
                            unselectedAddonIds.filter((id) => config.addons[id].requires?.includes(id) ?? false)
                                .length > 0
                        ) &&
                        selected,
                    ...rest,
...
const registrationInfoReducer = (
    ...
    switch (action.type) {
        ...
        case SubmitForm('register-ticket-day').type:
            return {
                ...state,
                ticketType: { type: 'day', day: DateTime.fromISO(action.payload.day!, { zone: 'Europe/Berlin' }) },
            }
...
const registrationReducer = (state: Registration, action: GetAction<AnyAppAction>): Registration => {
    switch (action.type) {
        default:
            return {
                ...state,
                registrationInfo: registrationInfoReducer(state.registrationInfo, action) as RegistrationInfo,
            }
    }
}
