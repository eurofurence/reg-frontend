...
import type { FormIds, FormValuesType } from '../forms'
...
import type { TicketLevelAddons } from '~/state/models/register'

type GetDefaultFormValuesFn = <F extends FormIds>(
  id: F
) => (s: AppState) => FormValuesType<F>

export const determineDefaultAddons = (
  ticketType: 'day' | 'full' | undefined
): TicketLevelAddons => {
  if (ticketType === undefined) {
    ...
    return map(
      (addon) => ({
        selected:
          addon.default &&
          !(addon.unavailableFor?.type?.includes(ticketType) ?? false),
        options: map((option) => option.default as never, addon.options),
      }),
      config.addons
    )
...
export const getDefaultFormValues = ((id: FormIds) =>
  (s: AppState): FormValuesType<FormIds> => {
    switch (id) {
...
type GetSubmittedFormValuesFn = <F extends FormIds>(
  id: F
) => (s: AppState) => FormValuesType<F> | undefined

// eslint-disable-next-line complexity
export const getSubmittedFormValues = ((id: FormIds) =>
  (s: AppState): FormValuesType<FormIds> | undefined => {
    switch (id) {
      case 'register-ticket-type': {
        const ticketType = getTicketType()(s)
      ...
      case 'register-ticket-day': {
        ...
        return ticketType === undefined || ticketType.type !== 'day'
          ? undefined
          : { day: ticketType.day.toISODate() }
      }
      ...
      case 'register-summary':
        return isEditMode()(s)
          ? { rulesAndConditionsAccepted: true }
          : undefined
